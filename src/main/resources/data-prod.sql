...
SELECT * FROM (VALUES
  ...
  ('JSON and API Data',     'ADVANCED', '{user: {id: 42, name: Alice Chen, email: alice@example.com}, settings: {theme: dark, fontSize: lg, notifications: true}, stats: {averageWpm: 67.3, lessonsCompleted: 18, totalRuns: 45, bestWpm: 82, percentile: 91}, certificates: [{tier: BASIC, issuedAt: 2026-01-15}, {tier: INTERMEDIATE, issuedAt: 2026-03-22}]}', 35, 50, 85.0),
  ('Shell Commands',        'ADVANCED', 'git log --oneline -20 | grep feat: && echo Features found || echo None cd ~/projects/typemaster && npm run build 2>&1 | tee build.log docker compose up -d --build && docker ps curl -s https://api.example.com/health | python3 -m json.tool find . -name *.java -exec grep -l TODO {} + ls -la /var/log/ | sort -k5 -rn | head -10 chmod 755 deploy.sh && ./deploy.sh', 36, 50, 85.0)
) AS v(title, difficulty_level, content_text, display_order, min_wpm, min_accuracy)
